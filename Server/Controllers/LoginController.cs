    ...
    public class LoginController : ControllerBase
    {
        ...
        public async Task<LoginResult> Login([FromBody] LoginModel login)
        {
            ...
            new Claim(ClaimTypes.Name, login.Email)
                ...
                return new LoginResult { Successful = true, Token = sendToken, Error = "" };
                //return Ok(new LoginResult { Successful = true, Token = new JwtSecurityTokenHandler().WriteToken(token), Error = "" });
            }
            catch (Exception ex)
            {
